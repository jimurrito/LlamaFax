...
def progBar(DBObj: Database, target=5000):
    cCT = DBObj.count()
    prog = cCT / target
        ...
        with st.form("ScoreBoard", clear_on_submit=True):
            ...
            for x in list(
                ...
                if ct == 1:
                    ...
                    Icon = ""
                st.write(f"[ {ct} ] - {x['upn']} - {x['score']} {Icon}")
                ct += 1
            st.write("_" * 3)
...
def STLogin(Image: str = "assets/lfx_profile_pic.jpg"):
    ...
    with Popout.container():
        ...
        st.title("LlamaFax Corpus Genorator")
        with st.form("AuthScreen"):  # , clear_on_submit=True
            ...
            Upn = st.text_input("Username/Email", max_chars=20)
...
def STsidebar(UserAuthObj, UAuthDB, Image: str = "assets/lfx_profile_pic.jpg"):
    ...
    with st.sidebar:
        ...
        with st.container():
            st.title("Hello, " + UserAuthObj["name"])
